...
- **DUART/LED-sign zero-read decode added (commit pending, this session)**: MAME's `itech020_map` (line 1021 of `itech32.cpp`) decodes `0x680800`–`0x68083f` ("Serial DUART Channel A/B & Top LED sign") as `readonly().nopw()` — a zero-initialised, read-only backing store (MAME zero-fills auto-allocated regions with no explicit init data), so real hardware/MAME always returns `0x0000` for reads there. Our FPGA had **no decode at all** for this range — reads fell through to the catch-all default of `16'hffff` (every status/ready bit permanently appearing SET). Given the ORANGE-confirmed deterministic block on real interrupts, this is a strong candidate root cause: if the ROM's boot code polls a DUART status/ready bit in this range waiting for it to clear before proceeding to enable interrupts, an all-1s read would make that poll spin forever on every single boot, independent of NVRAM or watchdog state — exactly matching the observed symptom. Added explicit `duart_cs` decode (`cpu_a[23:6]==18'h1_A020`, matching the exact 64-byte range) returning `16'h0000` on read, writes ignored (already a no-op for unmapped addresses, matching `.nopw()`). New standalone testbench `/tmp/tb_duart.v` (not committed) verifies the decode boundary exactly: `0x6807FE`/`0x680840` (just outside range) still read `0xffff`; `0x680800`/`0x68083E` (range boundaries) read `0x0000`. All five testbenches (`tb_masked`, `tb_isr`, `tb_ipl7`, `tb_wdogfired`, `tb_duart`) pass with no regressions. Not yet observed on real hardware — awaiting next flash. If this fixes it, expect GREEN (real interrupt finally taken); if not, still ORANGE/MAGENTA and the block lies elsewhere (ROM's own SR-mask/enable sequence never executing, or another unimplemented I/O range).

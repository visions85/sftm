...
SFTM is a work-in-progress FPGA core for **Street Fighter: The Movie** (Incredible Technologies itech32 arcade platform), built on [JTFRAME](https://github.com/jotego/jtcores) for the MiSTer FPGA target. Status: **SWAB=0 fix deployed (2026-07-26)** — the MRA sends 32-bit interleaved ROM data in little-endian byte order (prom0=D[7:0] first). `SWAB=1` reversed bytes within each 16-bit SDRAM word, producing a garbage reset-vector PC (a non-ROM address) so the CPU never fetched from ROM. Fixed by setting `SWAB=0` in `jtsftm_game_sdram.v`. Diagnostic: R=`!wdog_kick_ever`, G=`wdog_kick_ever`, B=`rom_cs_ever` (CPU entered ROM address space after boot_done). **Must load via MRA (not direct RBF) to re-download ROM with correct byte ordering.** Awaiting hardware observation (md5 `1b4291a2cf9d72c9aa02f0ccf0107cf2`). **MAGENTA confirmed (2026-07-26, build 3e0b7343)**: CPU entered ROM address space — `rom_cs_ever` latched. CPU IS executing real game code. Waiting to see if it transitions to GREEN (wdog_kick_ever=1 = main loop running). All RTL is Verilog (GPLv3) except TG68K.C (VHDL, LGPL), vendored as a git submodule at `cores/sftm/hdl/tg68k/`.

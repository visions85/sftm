...
- **SignalTap prep: `.stp` file built and verified, but scripted enable is structurally incompatible with jtcore's build methodology (this session, 2026-07-30)**: A USB Blaster clone was ordered (2-day shipping) to make real in-system capture possible; this entry covers the prep done while waiting for it to arrive, so the capture can start immediately once JTAG hardware exists.
  ...
  - **Practical path once the Blaster arrives**: gamingpc has a real desktop (KDE/Wayland, confirmed running) capable of hosting the Quartus GUI directly. The GUI's "Start Compilation" runs the whole flow as a single session (`execute_flow` within one process), not separate CLI invocations, and should not hit this specific issue. Import the already-verified `.stp` there (Assignments → Settings → SignalTap Logic Analyzer, or File → Open on the `.stp` directly), compile, program via the Blaster, and run the capture — no further scripting investigation needed first; the files and their node references are already validated correct.

- **Found where in the datapath to look: `genuine_exc_fetch_word=0x003C` is exactly byte-swap(low half), not random corruption — points at `jtframe_romrq_bcache`'s burst-reassembly shift register (this session, 2026-07-30)**: While waiting on the Blaster, did forensic analysis (verified precisely in Python, not by hand) of the captured fault word against the real ROM content. The real longword at `0x800410` is `0x158A3C00` (real ROM bytes: `15 8A 3C 00`); the CORRECT word there is `0x158A` (the high half). The captured `0x003C` is **exactly the byte-swapped LOW half** (`0x3C00` byte-swapped = `0x003C`) — not garbage, not an unrelated stale value, a precise deterministic transformation of data that genuinely belongs to the SAME longword.
  - **Why this matters**: `sftm_main.v`'s own port comment (`rom_half`, line ~505) documents that the 32-bit `rom_data` bus isn't a direct SDRAM read — it's assembled from TWO SEPARATE 16-bit SDRAM burst words by `jtframe_romrq_bcache` (vendored jtframe module, `modules/jtframe/hdl/sdram/jtframe_romrq_bcache.v`), via a two-stage `dst`/`dend` shift register: on the first burst word, `cached_data0[31:16] <= din`; on the second, `cached_data0[31:16] <= din` (new word) AND `cached_data0[15:0] <= cached_data0[31:16]` (shifting the FIRST word down into the low half). This is the **exact same module** where two real, previously-fixed bugs in this project already lived (`grom_cs` and `boot_cs` bcache-staleness, both documented earlier in this file) — both were cache-request/re-fetch-triggering bugs in this identical code area.
  - **A plausible concrete mechanism**: `dst` and `dend` are plain sequential `if` blocks in the same `always @(posedge clk)`, not mutually exclusive branches — if `dend` is still 1 (stale, left over from a previous, unrelated transaction) at the exact moment a new transaction's first word arrives and sets `dst`, BOTH blocks fire on the same edge. Since non-blocking assignments all read pre-edge values, `cached_data0[15:0] <= cached_data0[31:16]` would then load the low half from an **older, wrong cycle** instead of the word that actually belongs with the new high half. This would NOT show up in idealised or latency-swept RTL simulation (both already ruled out earlier this session) if those simulations' simplified ROM models never exercised bcache's real two-stage burst logic in the first place — every earlier "genuine_exc_fetch_word" simulation attempt used a direct address→data lookup function, not the real bcache state machine.
  - **Not proven — a hypothesis with a precise, falsifiable target**: this is the most concrete, specific lead found so far, but it is RTL analysis, not an observed fault. Real capture is still needed to confirm or refute it.
  - **New `sftm_bcache_fault.stp` staged alongside `sftm_ram_fault.stp`** (same trigger `genuine_exc_seen2`, same clock `emu:emu|clk_sys`, both copied into the project directory via `build_id.tcl`'s now-parameterized `copyStpFile`): probes `we`/`dst`/`dend`/`din[15:0]`/`double`/`good[1:0]`/`hit0`/`hit1`/`hitl[1:0]`/`req`/`cached_data0[31:0]`/`cached_addr0[16:0]`/`addr[16:0]`/`addr_ok`/`data_ok` directly inside `jtframe_romrq_bcache:u_block_cache` (confirmed real instance path `emu:emu|jtsftm_game_sdram:u_game|jtframe_rom_2slots:u_bank0|jtframe_romrq:u_slot0|jtframe_romrq_bcache:u_block_cache`, `DW=32`/`AW=17`/`SDRAMW=23` per the actual build's `map.rpt` parameter listing — `u_bank0` independently confirmed to be the program-ROM bank by tracing `main_addr`/`main_data`/`main_cs`/`main_ok` wiring in `jtsftm_game_sdram.v`), plus `cpu_addr` for context. 118 data bits, sample depth 1024. **Verified staged correctly**: both `.stp` files present with correct byte sizes in the project directory after a real build, and the build itself stays a clean 0-error baseline with both files staged (SignalTap still disabled per the entry above) — only the capture-and-arm step (needing the Blaster) remains.

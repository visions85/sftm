...
module jtframe_cache #(parameter
    ...
    input                   wr,
    input      [MW-1:0]     wdsn,
    output                  ok,       // held until the request deasserts
    input                   flush,
    output                  flushing,
    output                  flush_done,
    input                   invalidate,
...
// makes the handshake rate-agnostic: ok rises with the pulse and stays
// until the requester deasserts rd/wr, which every consumer already does
// right after sampling it.
wire ok_pulse;
reg  ok_hold;
always @(posedge clk) begin
    if( rst )                 ok_hold <= 1'b0;
    else if( ok_pulse )       ok_hold <= 1'b1;
    else if( !(rd || wr) )    ok_hold <= 1'b0;
end
assign ok = ok_pulse | ok_hold;

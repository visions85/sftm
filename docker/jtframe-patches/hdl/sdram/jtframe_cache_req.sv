...
always @(posedge clk) begin
    if( rst ) begin
        ...
        // a flush-read that misses its lookup window (the flush ends first)
        // must NOT stay pending: the requester's held rd re-triggers through
        // the normal path once the block lifts, and a stale pending would
        // fire a ghost lookup during the NEXT flush -- an ok with no request
        // holding, which mispairs the requester's handshake and drops the
        // following transaction. Found by tb_vramlane phase 7 (writes and
        // reads racing repeated flushes); on hardware it corrupted the full
        // GROM checksum test from the first build that used flush (b109).
        if( !flushing && !flush_rd_lookup ) flush_rd_pending <= 1'b0;

...
#
# Built on the printf-based TCL construction used by read-probe.sh. An earlier
# heredoc version failed with "There is already an active In-System Sources and
# Probes session" while read-probe.sh worked on the same hardware moments
# before -- the fault was the heredoc escaping, not the JTAG session.
#
# probe2 (id SFRN, 64 bit):
#   [63:56] 0x7E sig  [48] force_run echo  [47] m_got
#   [46:32] completed main fetches  [31:0] FIRST longword fetched
# The first longword must be 0x00008000 for SDRAM to have kept the ROM.
set -euo pipefail
FR="${1:-1}"; N="${2:-30}"
...
'puts \"P3 [read_probe_data -instance_index 2]\"' \
'write_source_data -instance_index 2 -value ${FR}' \
'after 500' \
'for { set i 0 } { \$i < ${N} } { incr i } { puts \"P3 [read_probe_data -instance_index 2]\"; after 60 }' \
'end_insystem_source_probe' > /tmp/p3.tcl
quartus_stp -t /tmp/p3.tcl 2>&1 | grep -E '^P3 |^instances:|ERROR'
"
